...
def process_questions(cfg, mode):
    ...
    if mode in ['train']:
        ...
        for i, instance in enumerate(instances):
            ...
            for sub_program in program:
                function = sub_program['function']
                if function not in program_token_to_idx:
                    program_token_to_idx[function] = len(program_token_to_idx)
        print('Get program_token_to_idx, num: %d' % len(program_token_to_idx))
    ...
    for idx, instance in enumerate(instances):
        ...
        if mode != 'test':
            ...
            functions = []
            for sub_program in program:
                functions.append(sub_program['function'])
            program_encoded = encode(functions, vocab['program_token_to_idx'], allow_unk=False)
            # answer
    ...
    if mode != 'test':
        ...
        for prog in programs_encoded:
            while len(prog) < max_program_length:
                prog.append(vocab['program_token_to_idx']['<NULL>'])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cfg', dest='cfg_file', help='optional config file', default='clevr.yml', type=str)

...
from DataLoader import CLEVRHumanDataLoader
...
def validate(cfg, model, data, device):
    ...
    with torch.no_grad():
        for batch in tqdm(data, total=len(data)):
            sorted_indices = np.argsort(-batch[3])
            for id_ in range(len(batch)):
                batch[id_] = batch[id_][sorted_indices]
            img_ids, answers, *batch_input = [todevice(x, device) for x in batch]
    ...
    if cfg.train.flag:
        ...
        return acc, results, gts, ctrl_head_1, ctrl_head_2, visual_adjs, ling_vis_binding, boxes, ids, questions
...
if __name__ == '__main__':
    ...
    if args.mode == 'val':
        ...
        val_loader = CLEVRHumanDataLoader(**val_loader_kwargs)
        model_kwargs.update({'vocab': val_loader.vocab})
        model = LOGNet(cfg, **model_kwargs).to(device)
        model.load_state_dict(loaded['state_dict'])

        valid_acc, preds, gts, ctrl_head_1, ctrl_head_2, visual_adjs, ling_vis_binding, boxes, ids, questions = validate(
        ...
        test_loader = CLEVRHumanDataLoader(**test_loader_kwargs)
